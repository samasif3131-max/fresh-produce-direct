...
function AuthProvider({ children }) {
    ...
    /* LOAD LOGGED IN USER */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "AuthProvider.useEffect": ()=>{
            const savedUser = localStorage.getItem("freshProduceLoggedInUser");
            if (savedUser) {
                try {
                    setUser(JSON.parse(savedUser));
                } catch  {
                    localStorage.removeItem("freshProduceLoggedInUser");
                }
            }
        }
    }["AuthProvider.useEffect"], []);
    /* REGISTER */ const register = (firstName, lastName, email, password)=>{
        ...
        /* SAVE ACCOUNT */ localStorage.setItem("freshProduceUser", JSON.stringify(newUser));
        /* REMOVE PASSWORD BEFORE SAVING SESSION */ const loggedInUser = {
            firstName: newUser.firstName,
            lastName: newUser.lastName,
            name: newUser.name,
            email: newUser.email
        };
        localStorage.setItem("freshProduceLoggedInUser", JSON.stringify(loggedInUser));
        setUser(loggedInUser);
        return true;
    };
    /* LOGIN */ const login = (email, password)=>{
        const savedAccount = localStorage.getItem("freshProduceUser");
        if (!savedAccount) {
            return false;
        }
        try {
            ...
            const cleanEmail = email.trim().toLowerCase();
            if (account.email.toLowerCase() === cleanEmail && account.password === password) {
                const loggedInUser = {
    ...
    /* LOGOUT */ const logout = ()=>{
        localStorage.removeItem("freshProduceLoggedInUser");
        localStorage.removeItem("freshProduceLoggedIn");
        localStorage.removeItem("freshProduceRememberMe");
        setUser(null);
    };
...
function WishlistProvider({ children }) {
    ...
    const removeFromWishlist = (name)=>{
        setWishlist((current)=>current.filter((item)=>item.name !== name));
    };
...
//# sourceMappingURL=_0s85qqd._.js.map